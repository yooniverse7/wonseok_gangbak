    ...
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < M; i++) {
            ...
            for (int j = 0; j < N; j++) {
                ...
                if (tomato[i][j] == 1) {
                    dq.offer(new int[] {i, j}); // 익은 토마토를 모두 큐에 넣음
                    visited[i][j] = true;
                } else if (tomato[i][j] == -1) {
                    visited[i][j] = true; // 토마토가 없는 곳도 방문 처리
                }
            }
        }
    ...
    public static void bfs(Deque<int[]> dq) {
        while (!dq.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                if (tomato[nextX][nextY] == 0) { // 안 익은 토마토를 발견하면
                    tomato[nextX][nextY] = tomato[nx][ny] + 1; // 익히고, 날짜 증가
